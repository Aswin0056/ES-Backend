...
// 🔹 **Fetch Last Expense**
app.get("/last-expense", authenticateUser, async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM expenses WHERE email = $1 ORDER BY created_at DESC LIMIT 1", [req.user.email]);
